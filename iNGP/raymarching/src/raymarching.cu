...
template <typename scalar_t>
__global__ void kernel_near_far_from_aabb_2(
    const scalar_t * __restrict__ rays_o,
    const scalar_t * __restrict__ rays_d,
    const scalar_t * __restrict__ aabb,
    const uint32_t N,
    ...
    const uint32_t xi = group_grid_idx / (group_grid_cnt * group_grid_cnt);
    const uint32_t yi = (group_grid_idx % (group_grid_cnt * group_grid_cnt)) / group_grid_cnt;
    const uint32_t zi = (group_grid_idx % (group_grid_cnt * group_grid_cnt)) % group_grid_cnt;
    const float group_grid_len = fabs(aabb[3] - aabb[0]) / group_grid_cnt;

    float new_aabb[6];
    new_aabb[0] = aabb[0] + group_grid_len * xi;
    new_aabb[1] = aabb[1] + group_grid_len * yi;
    new_aabb[2] = aabb[2] + group_grid_len * zi;
    new_aabb[3] = new_aabb[0] + group_grid_len;
    new_aabb[4] = new_aabb[1] + group_grid_len;
    new_aabb[5] = new_aabb[2] + group_grid_len;
    ...
    // get near far (assume cube scene)
    float near = (new_aabb[0] - ox) * rdx;
    float far = (new_aabb[3] - ox) * rdx;
    if (near > far) swapf(near, far);

    float near_y = (new_aabb[1] - oy) * rdy;
    float far_y = (new_aabb[4] - oy) * rdy;
    if (near_y > far_y) swapf(near_y, far_y);
    ...
    float near_z = (new_aabb[2] - oz) * rdz;
    float far_z = (new_aabb[5] - oz) * rdz;
    if (near_z > far_z) swapf(near_z, far_z);
...
template <typename scalar_t>
__global__ void kernel_march_rays_2(
    const uint32_t n_alive, 
    const uint32_t n_step, 
    const int* __restrict__ rays_alive, 
    const scalar_t* __restrict__ rays_t, 
...
void march_rays_2(const uint32_t n_alive, const uint32_t n_step, const at::Tensor rays_alive, const at::Tensor rays_t, const at::Tensor rays_o, const at::Tensor rays_d, const float bound, const float dt_gamma, const uint32_t max_steps, const uint32_t C, const uint32_t H, const at::Tensor grid, const at::Tensor near, const at::Tensor far, at::Tensor xyzs, at::Tensor dirs, at::Tensor deltas, at::Tensor noises) {
    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    rays_o.scalar_type(), "march_rays_2", ([&] {
        kernel_march_rays_2<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(), bound, dt_gamma, max_steps, C, H, grid.data_ptr<uint8_t>(), near.data_ptr<scalar_t>(), far.data_ptr<scalar_t>(), xyzs.data_ptr<scalar_t>(), dirs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), noises.data_ptr<scalar_t>());
    }));
}
...
template <typename scalar_t>
__global__ void kernel_composite_rays_2(
    const uint32_t n_alive, 
    const uint32_t n_step, 
    const float T_thresh,
    int* rays_alive, 
